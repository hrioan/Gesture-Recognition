function [] = skeletonViewer( skeleton, postFlag, framenum, joints, msg )
...
title(sprintf('%s Skeleton Frame: %d',msg, framenum));
hold on
for i = 1:length(SkeletonConnectionMap)
   
    % joint A
    X1 = skeleton(SkeletonConnectionMap(i,1),1);
    Y1 = skeleton(SkeletonConnectionMap(i,1),2);
    Z1 = skeleton(SkeletonConnectionMap(i,1),3);
    
    % Joint B
    X2 = skeleton(SkeletonConnectionMap(i,2),1);
    Y2 = skeleton(SkeletonConnectionMap(i,2),2);
    Z2 = skeleton(SkeletonConnectionMap(i,2),3);
